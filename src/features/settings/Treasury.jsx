...
const Treasury = ({ user }) => {
	...
	const anchorsUnknown = anchorsLoading || !!anchorsError;
	const mutationsDisabled = anchorsUnknown || cashUnavailable;
	...
	const handleAddAnchor = async (event) => {
		...
		if (mutationsDisabled || saving) return;
		setSaving(true);
		...
		if (result.success) {
			...
			flash(
				"No se pudo guardar el ancla. Revisa fecha, saldo y fuente.",
				"err",
			);
		}
	};
	...
	if (settingsLoading) {
		return (
			<div className="flex items-center justify-center py-12">
				...
				<span className="ml-3 text-[var(--color-fg-3)]">
					Preparando tesorería…
				</span>
			</div>
		);
	}
		...
		<div className="space-y-6">
			...
			<div className="rounded-md border border-[var(--color-line)] bg-[var(--color-bg-1)] px-6 py-5">
				...
				<p className="text-[13px] text-[var(--color-fg-3)] mb-4 max-w-2xl">
					Un ancla es un saldo bancario verificado en una fecha (extracto o
					DATEV). La caja actual se calcula desde el ancla más reciente más los
					movimientos importados posteriores.
				</p>
				...
				{anchorsUnknown && (
					<FinancialSourceStatus
						status={anchorsLoading ? "loading" : "error"}
						label="Lectura de anclas"
						onRetry={retryAnchors}
						retryLabel="Reintentar lectura de anclas"
					/>
				)}
							...
							>
								...
								<button
									...
									onClick={() => {
										...
										if (
											window.confirm(
												`¿Eliminar el ancla del ${formatDate(anchor.date)}?`,
											)
										) {
				...
				>
					<div>
						<label className={labelClass} htmlFor="anchor-date">
							Fecha
						</label>
						<input
							id="anchor-date"
							...
							max={todayIso()}
							onChange={(e) =>
								setAnchorForm((f) => ({ ...f, date: e.target.value }))
							}
							required
						/>
					</div>
					<div>
						<label className={labelClass} htmlFor="anchor-balance">
							Saldo verificado (€)
						</label>
						<input
							id="anchor-balance"
							...
							onChange={(e) =>
								setAnchorForm((f) => ({ ...f, balance: e.target.value }))
							}
							placeholder="1214.20"
							required
						/>
					...
					<div>
						<label className={labelClass} htmlFor="anchor-source">
							Fuente
						</label>
						<input
							id="anchor-source"
							...
							onChange={(e) =>
								setAnchorForm((f) => ({ ...f, source: e.target.value }))
							}
							placeholder="Extracto Volksbank / DATEV"
							required
						/>
					</div>
					<Button type="submit" disabled={saving || mutationsDisabled}>
						<Plus size={14} />
						Registrar ancla
					</Button>
				</form>
			...
			<div className="rounded-md border border-[var(--color-line)] bg-[var(--color-bg-1)] px-6 py-5">
				...
				<p className="text-[13px] text-[var(--color-fg-3)] mb-4 max-w-2xl">
					El IVA del mes M vence el día 10 del mes M+2 (Dauerfristverlängerung).
					Registra aquí el importe de cada Voranmeldung para que entre en el
					calendario y en el forecast.
				</p>
				...
				<form
					onSubmit={handleAddVat}
					className="grid grid-cols-1 md:grid-cols-3 gap-3 items-end"
				>
					<div>
						<label className={labelClass} htmlFor="vat-month">
							Mes
						</label>
						<input
							id="vat-month"
							...
							value={vatForm.month}
							onChange={(e) =>
								setVatForm((f) => ({ ...f, month: e.target.value }))
							}
							required
						/>
					</div>
					<div>
						<label className={labelClass} htmlFor="vat-amount">
							IVA a pagar (€)
						</label>
						<input
							id="vat-amount"
							...
							onChange={(e) =>
								setVatForm((f) => ({ ...f, amount: e.target.value }))
							}
			...
			<div className="rounded-md border border-[var(--color-line)] bg-[var(--color-bg-1)] px-6 py-5">
				...
				<p className="text-[13px] text-[var(--color-fg-3)] mb-4 max-w-2xl">
					Si la caja proyectada cae por debajo de este importe en las próximas
					semanas, Resumen muestra una alerta.
				</p>
				<div className="flex items-end gap-3 max-w-sm">
					<div className="flex-1">
						<label className={labelClass} htmlFor="alert-buffer">
							Importe (€)
						</label>
						<input
							id="alert-buffer"
					...
					<Button
						...
						disabled={
							saving || bufferDraft == null || String(bufferDraft).trim() === ""
						}
					>
						Guardar
					</Button>
