		...
		>
			<div className="mb-4 flex items-center justify-between">
				<div>
					...
					<p className="mt-2 font-display font-display text-[28px] font-medium tracking-tight text-[var(--color-fg-1)]">
						{value}
					</p>
				</div>
				<div
					className="flex h-11 w-11 items-center justify-center rounded-lg"
					style={{ backgroundColor: "var(--color-bg-1)", color: accent }}
				>
...
const OutlookCard = ({ title, balance, delta, accent, subtitle }) => (
	<div className="rounded-md border border-[var(--color-line)] bg-[var(--color-bg-1)] p-5">
		<p className="label-mono text-[var(--color-fg-4)]">{title}</p>
		<p
			className="mt-2 font-display text-[28px] font-medium tracking-tight"
			style={{ color: accent }}
		>
		...
		{delta != null && (
			<p
				className={`mt-2 text-sm font-medium ${delta >= 0 ? "text-[var(--color-ok)]" : "text-[var(--color-accent)]"}`}
			>
				{delta >= 0 ? "+" : ""}
...
const ProyeccionCashflow = ({ user }) => {
	...
	const forecastAvailable = forecast.available && !cashUnavailable;
	const cashLabel =
	...
	const alerts = useMemo(() => {
		...
		if (forecastAvailable && forecast.firstNegativeWeek) {
			items.push({
				type: "critical",
		...
		if (metrics.next14Net < 0) {
			items.push({
				type: "warning",
				text: `La ventana de 14 días ya muestra una presión neta de ${formatCurrency(metrics.next14Net)}.`,
			});
		}

		if (
			!cashUnavailable &&
			(metrics.runwayMonths || 0) > 0 &&
			metrics.runwayMonths < 2
		) {
			items.push({
				type: "critical",
				text: `La cobertura de caja estimada es menor a 2 meses (${metrics.runwayMonths.toFixed(1)} meses).`,
			});
		}
	...
	const collectionSlipBasis = forecastAvailable
		? formatCollectionSlipBasis(forecast.collectionSlip)
		: null;

	if ((metrics.loading || forecast.loading) && !cashUnavailable) {
		return (
	...
	return (
		<div className="space-y-6 pb-12">
				...
				<div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
					<div>
						...
						<h2 className="font-display text-[32px] font-medium tracking-tight text-[var(--color-fg-1)]">
							Horizonte de {forecast.horizonWeeks} semanas usando CXC, CXP, nómina,
							recurrentes e IVA.
						</h2>
						<p className="mt-3 max-w-3xl text-[15px] leading-7 text-[var(--color-fg-3)]">
							{forecastAvailable ? (
								<>
									Parte de la caja conciliada de hoy y solo mueve compromisos reales:
									facturas abiertas, obligaciones de nómina, costos recurrentes activos y
									estimados de IVA. Único supuesto: los cobros entran{" "}
									{forecast.collectionSlipDays} días después del vencimiento
									{collectionSlipBasis ? ` (${collectionSlipBasis})` : ""}.
								</>
							) : (
								"La proyección necesita una lectura de conciliación verificada. Los vencimientos de documentos siguen disponibles."
							)}
						</p>
					</div>
					<div className="rounded-lg border border-[var(--color-line-s)] bg-[var(--color-bg-1)] px-4 py-3">
						...
						<p className="mt-1 text-sm font-medium text-[var(--color-fg-1)]">
							Próximas {forecast.horizonWeeks} semanas
						</p>
					</div>
				</div>
			</section>
				...
				<div className="space-y-2">
					{alerts.map((alert) => (
						...
						>
							...
							/>
							<span
								className={`text-sm ${alert.type === "critical" ? "text-[var(--color-accent)]" : "text-[var(--color-warn)]"}`}
							>
								{alert.text}
							</span>
						</div>
			...
			<div className="grid gap-4 lg:grid-cols-4">
				<StatCard
					title="Caja actual"
					value={cashUnavailable ? cashLabel : formatCurrency(metrics.currentCash)}
					subtitle={
						cashUnavailable
							? "Saldo pendiente de verificación"
							: "Saldo bancario real a hoy"
					}
					accent={
						metrics.currentCash >= 0 ? "var(--color-fg-4)" : "var(--color-accent)"
					}
					icon={Wallet}
				/>
				<StatCard
					title="Ventana 14d"
					value={formatCurrency(metrics.next14Net)}
					subtitle={`${metrics.upcomingReceivables.length} cobros y ${metrics.upcomingPayables.length} pagos`}
					accent={metrics.next14Net >= 0 ? "var(--color-ok)" : "var(--color-warn)"}
					icon={Calendar}
				/>
				<StatCard
					title="Liquidez proyectada"
					value={
						cashUnavailable ? cashLabel : formatCurrency(metrics.projectedLiquidity)
					}
					subtitle="Caja actual + CXC abiertas - CXP abiertas"
					accent={
						cashUnavailable
							? "var(--color-fg-4)"
							: metrics.projectedLiquidity >= 0
								? "var(--color-ok)"
								: "var(--color-accent)"
					}
					icon={Target}
				...
				<StatCard
					...
					subtitle={`Egreso prom. mensual ${formatCurrency(metrics.avgMonthlyOutflows)}`}
				...
				<>
					<div className="grid gap-4 md:grid-cols-3">
						<OutlookCard
							...
							accent={
								forecast.endBalance >= 0 ? "var(--color-fg-4)" : "var(--color-accent)"
							}
						...
						<OutlookCard
							...
							delta={null}
							accent={
								(forecast.lowestWeek?.projectedBalance ?? 0) >= 0
									? "var(--color-fg-4)"
									: "var(--color-accent)"
							}
							subtitle={
								forecast.lowestWeek
									? `${forecast.lowestWeek.week} · ${forecast.lowestWeek.label}`
									: "Sin datos"
							}
						/>
						<OutlookCard
							title="Primera semana en negativo"
							balance={forecast.firstNegativeWeek?.projectedBalance ?? null}
							delta={null}
							accent={
								forecast.firstNegativeWeek ? "var(--color-accent)" : "var(--color-ok)"
							}
							subtitle={
								forecast.firstNegativeWeek
								...
								<tbody className="divide-y divide-[var(--color-line)]">
									{projectionData.map((row) => (
										<tr key={row.label} className="hover:bg-[var(--color-bg-1)]">
											<td className="px-4 py-3 font-medium text-[var(--color-fg-1)]">
												{row.label}
											</td>
											<td className="px-4 py-3 text-[var(--color-fg-3)]">{row.range}</td>
											<td className="px-4 py-3 text-right text-[var(--color-ok)]">
												{formatCurrency(row.committedIn)}
											</td>
											<td className="px-4 py-3 text-right text-[var(--color-accent)]">
												{formatCurrency(row.committedOut)}
											</td>
											<td
												className={`px-4 py-3 text-right ${row.net >= 0 ? "text-[var(--color-ok)]" : "text-[var(--color-accent)]"}`}
											>
												{formatCurrency(row.net)}
											</td>
											<td
												className={`px-4 py-3 text-right font-medium ${row.base >= 0 ? "text-[var(--color-fg-3)]" : "text-[var(--color-accent)]"}`}
											>
			...
			<div className="rounded-md border border-[var(--color-line-s)] bg-transparent p-5">
				<div className="flex items-start gap-3">
					...
					<div className="grid gap-3 md:grid-cols-3">
						<div>
							<p className="text-sm font-medium text-[var(--color-fg-3)]">
								Punto de partida
							</p>
							<p className="mt-1 text-sm text-[var(--color-fg-3)]">
								{forecastAvailable
									? `La caja conciliada de hoy (${formatCurrency(forecast.startBalance)}), la misma que usan el Resumen y las alertas operativas.`
									: "Caja pendiente de verificación; no se calcula un saldo de partida."}
							</p>
						</div>
						<div>
							<p className="text-sm font-medium text-[var(--color-fg-3)]">
							...
							<p className="mt-1 text-sm text-[var(--color-fg-3)]">
								Solo compromisos reales: CXC y CXP abiertas, obligaciones de nómina,
								costos recurrentes activos y estimados de IVA. No se asumen ventas ni
								compras futuras.
							</p>
						</div>
						<div>
							...
							<p className="mt-1 text-sm text-[var(--color-fg-3)]">
								{forecastAvailable
									? `${formatCollectionSlip(forecast.collectionSlip)}; lo ya vencido se espera de inmediato. El plazo sale del histórico de cobros de la empresa, no de un porcentaje inventado.`
									: "El plazo de cobro no está disponible mientras la proyección no se pueda calcular."}
							</p>
						</div>
					</div>
				</div>
