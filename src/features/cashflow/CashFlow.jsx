...
const CashFlow = ({ user }) => {
	...
	const vatCoverage = useMemo(() => {
		...
		const total = derived.reduce(
			(sum, entry) => sum + (entry.totalAmount || 0),
			0,
		);
		const known = derived.reduce(
			(sum, entry) => sum + (entry.knownAmount || 0),
			0,
		);
	...
	const recentMovements = [...metrics.filteredMovements]
		.sort((left, right) =>
			(right.postedDate || "").localeCompare(left.postedDate || ""),
		)
		.slice(0, 12);
	...
	return (
		<div className="space-y-6 pb-12">
			...
			<KPIGrid cols={2}>
				<KPI
					...
					value={formatCurrency(
						metrics.upcomingReceivables.reduce(
				...
				<KPI
					...
					value={formatCurrency(
						metrics.upcomingPayables.reduce(
							(sum, entry) => sum + entry.openAmount,
							0,
						),
					)}
			...
			<p className="text-[12px] text-[var(--color-fg-4)]">
				{cashUnavailable
					? "Saldo de caja pendiente de verificación. Los movimientos contabilizados se conservan."
					: "Caja conciliada: último ancla + movimientos bancarios posteriores."}
			</p>

			<Section
				title="Estado de Resultados"
				subtitle="Ingresos vs gastos realizados, agrupados por mes."
				help={
					<HelpButton title="Estado de Resultados" size={14}>
						<p>Resumen de ingresos y gastos reales de los ultimos 6 meses.</p>
						<p>
							Solo incluye movimientos contabilizados (cobros y pagos
							ejecutados), no compromisos pendientes.
						</p>
					</HelpButton>
				}
			>
					...
					<table className="w-full text-left text-sm">
						<thead>
							<tr className="border-b border-[var(--color-line)]">
								<th className="px-3 py-2.5 label-mono text-[var(--color-fg-3)]">
									Mes
								</th>
								<th className="px-3 py-2.5 text-right label-mono text-[var(--color-fg-3)]">
									Ingresos
								</th>
								<th className="px-3 py-2.5 text-right label-mono text-[var(--color-fg-3)]">
									Gastos
						...
						<tbody className="divide-y divide-[var(--color-line)]">
							{monthlyPL.map((row) => (
								<tr key={row.ym} className="hover:bg-[var(--color-bg-1)]">
									...
									<td className="px-3 py-3 text-right text-[13px] font-medium text-[var(--color-ok)]">
									...
									<td
										className={`px-3 py-3 text-right text-[13px] font-medium ${row.net >= 0 ? "text-[var(--color-ok)]" : "text-[var(--color-err)]"}`}
									>
										<span className="inline-flex items-center justify-end gap-2">
											<Badge variant={row.net >= 0 ? "ok" : "err"} dot>
												{row.net >= 0 ? "Positivo" : "Negativo"}
											</Badge>
			...
			<div className="grid gap-6 xl:grid-cols-[1.1fr,0.9fr]">
				<Section
					title="Balance de caja semanal"
					subtitle="Historico reciente derivado de movimientos contabilizados."
					help={
						<HelpButton title="Balance de caja semanal" size={14}>
							<p>
								Evolucion del saldo bancario semana a semana, basado en
								movimientos reales.
							</p>
							<p>
								Permite detectar tendencias de consumo o acumulacion de
								liquidez.
							</p>
						</HelpButton>
					}
				>
					{cashUnavailable ? (
						<p className="text-sm text-[var(--color-fg-3)]">
							Balance de caja no disponible hasta verificar la conciliación.
						</p>
					) : (
				...
				<Section
					title="Compromisos por semana"
					subtitle={`Entradas y salidas comprometidas en la siguiente ventana de ${forecast.horizonWeeks} semanas.`}
					help={
						<HelpButton title="Compromisos por semana" size={14}>
							<p>
								Cobros, pagos, nomina, costos recurrentes e IVA agrupados por
								semana.
							</p>
							<p>
								Sale de la misma proyeccion que usan el Resumen y la vista de
								Proyeccion.
							</p>
							{forecastAvailable && (
				...
				>
					{!forecastAvailable ? (
						<p className="text-sm text-[var(--color-fg-3)]">
							Compromisos proyectados no disponibles hasta verificar la
							conciliación.
						</p>
					) : (
						<>
			...
			<Section
				...
				help={
					<HelpButton title="IVA por liquidar" size={14}>
						<p>
							IVA repercutido de las facturas emitidas menos IVA soportado de
							los pagos ya clasificados.
						</p>
						<p>
							Los meses marcados como Manual vienen de Configuracion → Tesoreria
							y mandan sobre el calculo.
						</p>
						<p>
							La cobertura dice que parte de los importes tiene tipo de IVA
							configurado.
						</p>
					</HelpButton>
				}
			>
				{!forecastAvailable ? (
					<p className="text-sm text-[var(--color-fg-3)]">
						IVA proyectado no disponible; no se puede concluir que no haya
						obligaciones.
					</p>
					...
					<div className="rounded-lg border border-dashed border-[var(--color-line)] px-4 py-10 text-center text-sm text-[var(--color-fg-3)]">
						Sin IVA estimado por liquidar: no hay facturas emitidas ni pagos
						clasificados en los meses todavía sin declarar.
					</div>
				) : (
					<>
						<div className="overflow-x-auto">
							<table className="w-full text-left text-sm">
								...
								<tbody className="divide-y divide-[var(--color-line)]">
									{vatUpcoming.map((entry) => (
										<tr
											key={entry.month}
											className="hover:bg-[var(--color-bg-1)]"
										>
											<td className="px-3 py-3 text-[13px] font-medium text-[var(--color-fg-1)]">
												{monthLabel(entry.month)}
												<span
													className={`nx-badge ml-2 ${entry.source === "manual" ? "nx-badge-info" : "nx-badge-neutral"}`}
												>
													{entry.source === "manual" ? "Manual" : "Estimado"}
												</span>
											</td>
											<td className="px-3 py-3 text-[13px] text-[var(--color-fg-3)]">
			...
			<div className="grid gap-6 xl:grid-cols-[1.05fr,0.95fr]">
				<div ref={movementsRef}>
					<Section
						...
						subtitle="Ultimas entradas y salidas contabilizadas en la cuenta principal."
					>
										...
										<div className="flex items-center gap-3">
											...
											<div>
												...
												<p className="text-xs text-[var(--color-fg-3)]">
													{movement.counterpartyName || "Sin contraparte"} ·{" "}
													{formatDate(movement.postedDate)}
												</p>
											</div>
										</div>
										<div className="text-right">
											<p
												className={`text-sm font-medium ${isInflow ? "text-[var(--color-ok)]" : "text-[var(--color-warn)]"}`}
											>
												{isInflow ? "+" : "-"}
				...
				<div ref={reconciliationRef}>
					<Section
						title="Pendiente de conciliacion"
						subtitle="Movimientos bancarios aun no vinculados a un cierre mensual."
					>
						<div className="space-y-3">
							...
							{metrics.unreconciledMovements.slice(0, 10).map((movement) => (
								...
								>
									<div className="mb-2 flex items-center justify-between gap-3">
										...
										<span className="text-sm font-medium text-[var(--color-fg-1)]">
											{formatCurrency(movement.amount)}
										</span>
									</div>
									<p className="text-xs text-[var(--color-fg-3)]">
			...
			<div className="grid gap-6 lg:grid-cols-2">
				...
				>
					<Section
						...
						subtitle="Documentos abiertos con vencimiento pasado."
					>
						<div className="flex items-center gap-4">
							...
							<div>
								...
								<p className="text-sm text-[var(--color-fg-3)]">
									{formatCurrency(
										metrics.overdueReceivables.reduce(
											(sum, entry) => sum + entry.openAmount,
											0,
										),
									)}
				...
				>
					<Section
						title="Pagos por salir"
						subtitle="Compromisos abiertos dentro de la siguiente ventana."
					>
						<div className="flex items-center gap-4">
							<div className="flex h-12 w-12 items-center justify-center rounded-md border border-[var(--color-line)] text-[var(--color-warn)]">
								<Clock3 size={18} />
			...
			<Section
				...
				subtitle="Referencia rápida para la operación diaria."
			>
				<div className="grid gap-4 md:grid-cols-3">
					<div className="rounded-lg border border-[var(--color-line-s)] bg-[var(--color-bg-1)] px-4 py-4">
						<div
							className={`mb-2 flex items-center gap-2 ${cashUnavailable ? "text-[var(--color-fg-3)]" : "text-[var(--color-ok)]"}`}
						>
							{cashUnavailable ? (
								<ShieldAlert size={16} />
							) : (
							...
							<span className="text-sm font-medium">
								{cashUnavailable
									? "Caja pendiente de verificación"
									: "Caja registrada"}
							</span>
						</div>
						<p className="text-sm leading-6 text-[var(--color-fg-3)]">
							{cashUnavailable
								? "Los movimientos confirmados siguen visibles, pero el saldo de caja no está disponible."
								: "Los movimientos confirmados alimentan el saldo disponible y la conciliación."}
						</p>
					</div>
					<div className="rounded-lg border border-[var(--color-line-s)] bg-[var(--color-bg-1)] px-4 py-4">
						...
						<p className="text-sm leading-6 text-[var(--color-fg-3)]">
							Las facturas abiertas se siguen por separado hasta que el cobro o
							el pago ocurre.
						</p>
					</div>
					<div className="rounded-lg border border-[var(--color-line-s)] bg-[var(--color-bg-1)] px-4 py-4">
						...
						<p className="text-sm leading-6 text-[var(--color-fg-3)]">
							Revisar y conciliar cada semana mejora la calidad del cierre y de
							la proyección.
						</p>
					</div>
				</div>
