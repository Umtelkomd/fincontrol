...
describe("Resumen — cockpit render", () => {
	...
	it("retains independent data through reconciliation loading, failure, retry and fresh recovery", () => {
		...
		const independent = screen
			...
			.closest("section");
		expect(within(independent).getByText("10.000,00")).toBeInTheDocument();
		fireEvent.click(
		...
		act(() =>
			listeners[1].next({
				exists: () => true,
				data: () => store.documents.reconciliation,
			}),
		);
		expect(within(cashKpi()).getByText("52.000,00")).toBeInTheDocument();
		expect(screen.getByTestId("position-net")).toHaveTextContent("58.000,00");
		expect(screen.getByText("Por cobrar / por pagar").closest("section")).toBe(
			independent,
		);
		act(() => listeners[1].fail(new Error("later failure")));
		expect(within(cashKpi()).queryByText("52.000,00")).not.toBeInTheDocument();
		fireEvent.click(
			screen.getByRole("button", { name: "Reintentar conciliación" }),
		);
...
describe("Resumen — alerts panel", () => {
	...
	it("surfaces the overdue payables and receivables alerts", () => {
		...
		const panel = screen.getByText("Alertas").closest("section");
		expect(
			within(panel).getByText("Cuentas por pagar vencidas"),
		).toBeInTheDocument();
		expect(
			within(panel).getByText("Cobros vencidos sin gestionar"),
		).toBeInTheDocument();
	...
	it("links a stale reconciliation anchor to Configuración", () => {
		...
		const panel = screen.getByText("Alertas").closest("section");
		const stale = within(panel)
			.getByText("Conciliación desactualizada")
			.closest("li");
		expect(within(stale).getByRole("link")).toHaveAttribute(
			"href",
			"/configuracion",
		);
	});
...
describe("Resumen — partial data guard", () => {
	it("warns that the figures are untrustworthy when a source fails", () => {
		...
		expect(
			screen.getByText("Datos incompletos — no confíes en estas cifras"),
		).toBeInTheDocument();
		expect(
			screen.getByText(/No se pudieron cargar: movimientos bancarios/),
		).toBeInTheDocument();
	});
});
...
describe("Resumen — payroll permission gating", () => {
	it("drops the payroll figures for a role without the cxp permission", () => {
		...
		expect(
			screen.queryByText("Resultado del mes (nómina incluida)"),
		).not.toBeInTheDocument();
		expect(screen.getByText("Sin permiso")).toBeInTheDocument();
		expect(
			screen.getByText("Sin mano de obra (sin permiso)"),
		).toBeInTheDocument();
	});
});
...
describe("Resumen — posición neta", () => {
	...
	it("keeps the headline on the shared formula and states executed work as its own line", () => {
		...
		expect(within(panel).getByTestId("position-net")).toHaveTextContent(
			"58.000,00",
		);
	...
	it("still renders the position when nothing is captured yet", () => {
		...
		expect(
			within(panel).queryByTestId("position-with-wip"),
		).not.toBeInTheDocument();
		expect(
			within(panel).getByText(/Sin obra ejecutada registrada/),
		).toBeInTheDocument();
	});

	it("says plainly when the executed work has been frozen by paperwork", () => {
		store.collections.workInProgress = [
			{ ...WIP_DOC, asOf: isoDaysAgoLocal(80) },
		];

		renderScreen(<Resumen user={USER} />);

		expect(
			within(screen.getByTestId("position-panel")).getByText(/80 días/),
		).toBeInTheDocument();
	});
