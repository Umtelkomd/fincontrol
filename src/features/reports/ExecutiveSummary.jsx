...
const sumOpen = (rows) =>
	rows.reduce((sum, entry) => sum + entry.openAmount, 0);
...
const ExecutiveSummary = ({ user }) => {
	...
	const alerts = [
		{
			...
			body: cashUnavailable
				? `${cashLabel}: caja y posición neta pendientes de conciliación.`
				: `Caja ${formatCurrency(metrics.currentCash)} y posición neta ${formatCurrency(metrics.netPosition)}.`,
			tone: cashUnavailable
				? "warning"
	...
	const recommendations = [
		cashUnavailable
			? "Recuperar la conciliación antes de tomar decisiones sobre caja o proyección."
		...
		<div className="space-y-6">
			...
			<section className="rounded-md border border-[var(--color-line)] bg-[var(--color-bg-1)] p-5 ">
				<div className="mb-5">
					...
					<p className="mt-1 text-sm text-[var(--color-fg-3)]">
						Resumen del estado operativo financiero a partir de la operación
						registrada.
					</p>
				</div>
				<div className="grid gap-4 lg:grid-cols-3">
					{alerts.map((alert) => (
						...
						>
							...
							<p className="mt-2 text-sm leading-7 text-[var(--color-fg-4)]">
								{alert.body}
							</p>
						</div>
					))}
			...
			<section className="grid gap-6 lg:grid-cols-[1.05fr,0.95fr]">
				<div className="rounded-md border border-[var(--color-line)] bg-[var(--color-bg-1)] p-5 ">
					<div className="mb-5">
						<h3 className="text-[18px] font-medium tracking-tight text-[var(--color-fg-1)]">
							Frente de vencimientos
						</h3>
						<p className="mt-1 text-sm text-[var(--color-fg-3)]">
							Prioridades inmediatas de caja.
						</p>
					</div>
					<div className="space-y-3">
						{[
							...metrics.upcomingReceivables.slice(0, 3),
							...metrics.upcomingPayables.slice(0, 3),
						].map((entry) => {
							const isInflow = entry.kind === "receivable";
							return (
								<div
									key={entry.id}
									className="rounded-md border border-[var(--color-line)] bg-[var(--color-bg-1)] px-4 py-4"
								>
				...
				<div className="rounded-md border border-[var(--color-line)] bg-[var(--color-bg-1)] p-5 ">
					<div className="mb-5">
						...
						<p className="mt-1 text-sm text-[var(--color-fg-3)]">
							Acciones sugeridas para la siguiente semana operativa.
						</p>
					</div>
					<div className="space-y-3">
						{recommendations.map((recommendation) => (
							<div
								...
								className="rounded-md border border-[var(--color-line)] bg-[var(--color-bg-1)] px-4 py-4"
							>
								<p className="text-sm leading-7 text-[var(--color-fg-4)]">
									{recommendation}
								</p>
							</div>
						))}
