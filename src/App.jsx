...
} from "./contexts/FinanceLedgerContext";
...
const ReportesUnified = lazy(
	() => import("./features/reportes/ReportesUnified"),
);
const ConfiguracionUnified = lazy(
	() => import("./features/configuracion/ConfiguracionUnified"),
);
const CXCIndependiente = lazy(() => import("./features/cxc/CXCIndependiente"));
const BatchReconciliation = lazy(
	() => import("./features/cxc/BatchReconciliation"),
);
const CXPIndependiente = lazy(() => import("./features/cxp/CXPIndependiente"));
const BudgetVsActual = lazy(
	() => import("./features/presupuesto/BudgetVsActual"),
);
const AuditLog = lazy(() => import("./features/auditoria/AuditLog"));
const ProyectoDashboard = lazy(
	() => import("./features/proyectos/ProyectoDashboard"),
);
const ProyeccionCashflow = lazy(
	() => import("./features/cashflow/ProyeccionCashflow"),
);
...
function AppContent({ user, userRole, hasPermission }) {
	...
	const bankBalanceData =
		ledger.loading || cashUnavailable
			? null
			: {
	...
	return (
		<div className="relative flex h-full flex-col overflow-hidden bg-[var(--color-bg-0)] font-sans text-[14px] text-[var(--color-fg-1)]">
			<div
				aria-hidden="true"
				className="fixed inset-x-0 top-0 z-[300] h-[3px] bg-[var(--color-accent)]"
			/>
			<Sidebar
				user={user}
				userRole={userRole}
				...
				<div className="flex-1 overflow-y-auto px-4 pb-8 pt-5 md:px-8 md:pb-10 md:pt-6">
					<Suspense fallback={<LoadingState />}>
						<Routes>
							...
							<Route
								path="/perfil"
								element={<UserProfile user={user} userRole={userRole} />}
							/>
							<Route
								path="/empleados"
								element={
