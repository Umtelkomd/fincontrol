...
const mutatorsOf = (hook) =>
	Object.fromEntries(
		Object.entries(hook).filter(([, value]) => typeof value === "function"),
	);
...
export const useFinanceLedger = (rawUser) => {
	...
	const reconciliationHook = useReconciliation(user);
	const {
		anchors,
	...
	const currentActions = {
		...
		reconciliation: mutatorsOf(reconciliationHook),
	};
	...
	return useMemo(() => {
		...
		const sourceErrors = {
			...
			reconciliation: anchorsError || null,
		};
		const error =
			movementError ||
			accountError ||
		...
		const canonicalReceivables = receivables.map((entry) =>
			adaptReceivableDoc(entry, "receivable"),
		);
		const canonicalPayables = payables.map((entry) =>
			adaptPayableDoc(entry, "payable"),
		);
			...
			sumMoney(
				postedMovements.filter(
					(entry) =>
						...
						compareIsoDate(entry.postedDate, mainAccount.openingDate) > 0,
				),
				getSignedMovementAmount,
		...
		const {
			...
			reconciliationLoading: anchorsLoading,
			reconciliationError: anchorsError,
		});
		...
		const availableCredit =
			currentBalance === null
				? null
				: currentBalance - mainAccount.creditLineLimit;
		...
		return {
			...
			summary: {
				...
				creditUsed,
				availableCredit,
				pendingReceivables: receivableRows.reduce(
					(sum, entry) => sum + entry.openAmount,
					0,
				),
				pendingPayables: payableRows.reduce(
					(sum, entry) => sum + entry.openAmount,
					0,
				),
			},
