...
describe("useTreasuryMetrics — unavailable cash", () => {
	...
	it.each([
		...
		NaN,
		Infinity,
	])("keeps independent metrics but never coerces %s cash to zero", (cash) => {
		const ledger = ledgerWith({
			cash,
			receivables: [document({ openAmount: 500 })],
			payables: [document({ openAmount: 200 })],
		});
		ledger.cashSource = "unavailable";
		ledger.cashMeta.status = "error";
		ledger.sourceErrors.reconciliation = new Error("synthetic failure");
		const metrics = metricsFor(ledger);
		expect(metrics.currentCash).toBeNull();
		expect(metrics.netPosition).toBeNull();
		expect(metrics.projectedLiquidity).toBeNull();
		expect(metrics.runwayMonths).toBeNull();
		expect(metrics.cashSeries).toEqual([]);
		expect(metrics.pendingReceivables).toBe(500);
		expect(metrics.pendingPayables).toBe(200);
		expect(metrics.overdueReceivables).toHaveLength(1);
		expect(metrics.cashInflows).toBe(0);
		expect(metrics.sourceErrors).toBe(ledger.sourceErrors);
	});
});
...
describe("useTreasuryMetrics — netPosition ignores the movement range", () => {
	...
	it("produces the same figure with and without a range", () => {
		...
		expect(ranged.upcomingReceivables.map((entry) => entry.id)).toEqual([
			"cxc-blank",
		]);
	});
...
describe("useTreasuryMetrics — movement totals still respect the range", () => {
	...
	it("sums inflows and outflows inside from/to only", () => {
		...
		expect(metrics.filteredMovements.map((entry) => entry.id)).toEqual([
			"in-2026",
			"out-2026",
		]);
