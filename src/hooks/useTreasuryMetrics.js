...
const buildCashSeries = (ledger, referenceDate) => {
		...
		sumMoney(
			ledger.postedMovements.filter(
				(entry) =>
					...
					compareIsoDate(entry.postedDate, ledger.bankAccount.openingDate) >
						0 &&
	...
	for (let index = 0; index < 12; index += 1) {
		...
		series.push({
			...
			inflows: sumMoney(
				bucketMovements.filter((entry) => entry.direction === "in"),
				(entry) => entry.amount,
			),
			outflows: sumMoney(
				bucketMovements.filter((entry) => entry.direction === "out"),
				(entry) => entry.amount,
			),
			net,
		});
...
const UNASSIGNED_PROJECT_LABELS = new Set([
	"",
	"sin proyecto",
	"sin asignar",
	"n/a",
 ...
 */
export const buildUnassignedMargin = (
	movements = [],
	payrollByProject = {},
) => {
...
export const useTreasuryMetrics = (options = {}) => {
	...
	return useMemo(() => {
		...
		const matchProject = (entry) =>
			!projectId ||
			entry.projectId === projectId ||
			entry.projectName === projectId;
		...
		const receivablesAgingView = buildAgingView({
			docs: openReceivables,
			today: referenceDate,
		});
		const payablesAgingView = buildAgingView({
			docs: openPayables,
			today: referenceDate,
		});
		...
		const upcomingReceivables = openReceivables.filter((entry) => {
			...
			return (
				iso &&
				compareIsoDate(iso, toISODate(referenceDate)) >= 0 &&
				compareIsoDate(iso, toISODate(nextWindowEnd)) <= 0
			);
		});
		...
		const cashInflows = sumMoney(
			filteredMovements.filter((entry) => entry.direction === "in"),
			(entry) => entry.amount,
		);
		const cashOutflows = sumMoney(
			filteredMovements.filter((entry) => entry.direction === "out"),
			(entry) => entry.amount,
		);
		...
		const monthsCovered2026 = Math.max(
			1,
			Math.min(
				3,
				Math.ceil(
					(new Date(referenceDate) - new Date(year2026Start)) /
						(1000 * 60 * 60 * 24 * 30),
				),
			),
		);
		const avgMonthlyOutflows = clampMoney(
			sumMoney(trailingOutflows, (entry) => entry.amount) / monthsCovered2026,
		);
		const avgMonthlyInflows = clampMoney(
			sumMoney(trailingInflows, (entry) => entry.amount) / monthsCovered2026,
		);
		...
		const runwayMonths = !cashAvailable
			? null
			: currentCash <= 0
				? 0
		...
		const netPosition = cashAvailable
			? clampMoney(currentCash + pendingReceivables - pendingPayables)
			: null;
		...
		return {
			...
			avgMonthlyInflows,
			avgMonthlyOutflows,
			cashSeries: cashAvailable ? buildCashSeries(ledger, referenceDate) : [],
			...
			projectMargins: buildProjectMargins(
				filteredMovements,
				options.payrollByProject || {},
			),
			unassignedMargin: buildUnassignedMargin(
				filteredMovements,
				options.payrollByProject || {},
			),
			unreconciledMovements: ledger.bankMovements
				...
				.sort((left, right) =>
					compareIsoDate(right.postedDate, left.postedDate),
				),
		};
	}, [
		accountId,
