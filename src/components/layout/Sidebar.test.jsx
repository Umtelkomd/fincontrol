...
describe("Sidebar — group tabs", () => {
  ...
  it("derives the active group from the route, on path boundaries", () => {
    ...
    expect(
      within(groupTabs()).getByRole("tab", { name: "Operar" }),
    ).toHaveAttribute("aria-selected", "true");
    expect(
      within(groupTabs()).getByRole("tab", { name: "Ver" }),
    ).toHaveAttribute("aria-selected", "false");
    // Row 2 shows only the operativo items.
    expect(
      within(itemRow()).getByRole("button", { name: /Bandeja/ }),
    ).toBeInTheDocument();
    expect(
      within(itemRow()).queryByRole("button", { name: /Presupuesto/ }),
    ).not.toBeInTheDocument();
    expect(
      within(itemRow()).getByRole("button", { name: /CXC/ }),
  ...
  it("shows the items of the report group when a report route is open", () => {
    ...
    expect(
      within(itemRow()).getByRole("button", { name: /Reportes/ }),
    ).toBeInTheDocument();
    expect(
      within(itemRow()).queryByRole("button", { name: /Bandeja/ }),
    ).not.toBeInTheDocument();
  });
  ...
  it("navigates to the first permitted item of a group when its tab is clicked", () => {
    ...
    expect(
      within(itemRow()).getByRole("button", { name: /Proyectos/ }),
    ).toBeInTheDocument();
  });
  ...
  it("hides the groups and items the role cannot open", () => {
    ...
    expect(
      within(itemRow()).getByRole("button", { name: /Resumen/ }),
    ).toBeInTheDocument();
    expect(
      within(itemRow()).queryByRole("button", { name: /Bandeja/ }),
    ).not.toBeInTheDocument();
    expect(
      within(itemRow()).queryByRole("button", { name: /Banco/ }),
    ).not.toBeInTheDocument();
  });

  it("shows Banco and Bandeja in Operar for a manager-like checker", () => {
    const managerHas = (section) =>
      ["bank", "dashboard", "cxp", "cxc", "reports"].includes(section);
    renderSidebar({ hasPermission: managerHas, userRole: "manager" });

    expect(
      within(itemRow()).getByRole("button", { name: /Banco/ }),
    ).toBeInTheDocument();
    expect(
      within(itemRow()).getByRole("button", { name: /Bandeja/ }),
    ).toBeInTheDocument();
  ...
  it("keeps Banco in Operar and out of Configuración for admin", () => {
    renderSidebar();
    ...
    fireEvent.click(
      within(groupTabs()).getByRole("tab", { name: "Configuración" }),
    );
