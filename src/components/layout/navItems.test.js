...
const groups = [
  ...
  },
  {
    ...
    label: "Configuración",
    items: [
      { path: "/configuracion", label: "Config", permission: "settings" },
    ],
  },
];
...
describe("activeGroupKey", () => {
  ...
  it("uses the real groups by default", () => {
    ...
    expect(activeGroupKey("/banco")).toBe("operativo");
  });
});
...
describe("visibleNavGroups", () => {
  it("keeps only the items the user may open and drops empty groups", () => {
    const visible = visibleNavGroups(
      (section) => section === "dashboard" || section === "reports",
      groups,
    );

    expect(visible.map((group) => group.key)).toEqual([
      "operativo",
      "reportes",
    ]);
  ...
  it("keeps items with no permission requirement", () => {
    const open = [
      { key: "x", label: "X", items: [{ path: "/perfil", label: "Perfil" }] },
    ];
  ...
  it("shows Banco and Bandeja under Operar for a manager-like checker", () => {
    const managerHas = (section) =>
      ["bank", "dashboard", "cxp", "cxc", "reports"].includes(section);
    const visible = visibleNavGroups(managerHas);
    const operarPaths = visible
      .find((group) => group.key === "operativo")
      .items.map((item) => item.path);
    const allPaths = visible.flatMap((group) =>
      group.items.map((item) => item.path),
    );

    expect(operarPaths.indexOf("/banco")).toBe(
      operarPaths.indexOf("/resumen") + 1,
    );
    expect(operarPaths.indexOf("/clasificar")).toBe(
      operarPaths.indexOf("/banco") + 1,
    );
    expect(allPaths).not.toContain("/empleados");
    expect(allPaths).not.toContain("/reglas");
  });

  it("hides Banco and Bandeja from an editor-like checker", () => {
    const visible = visibleNavGroups((section) => section === "dashboard");
    const operarPaths = visible
      .find((group) => group.key === "operativo")
      .items.map((item) => item.path);

    expect(operarPaths).toContain("/resumen");
    expect(operarPaths).not.toContain("/banco");
    expect(operarPaths).not.toContain("/clasificar");
  });
});
