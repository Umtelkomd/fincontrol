...
describe("LiquidityKpis", () => {
	it.each([
		"loading",
		"error",
	])("does not turn unavailable %s cash into zero or runway advice", (status) => {
		const { rerender } = render(
			<LiquidityKpis
				...
				forecast={{ available: false, status }}
			/>,
		);
		expect(screen.queryByText("0,00")).not.toBeInTheDocument();
		expect(screen.queryByText("Bajo cero")).not.toBeInTheDocument();
		expect(
			screen.getAllByText(status === "loading" ? "Cargando…" : "No disponible"),
		).toHaveLength(3);
		rerender(<LiquidityKpis metrics={metricsFixture()} />);
		expect(screen.getByText("52.000,00")).toBeInTheDocument();
		expect(screen.getByText("58.000,00")).toBeInTheDocument();
	});
	it("keeps healthy zero cash valid and does not infer runway from an unavailable forecast", () => {
		const { rerender } = render(
			<LiquidityKpis
				metrics={metricsFixture({ currentCash: 0, netPosition: 0 })}
			/>,
		);
		...
		rerender(
			<LiquidityKpis
				metrics={metricsFixture()}
				forecast={{ available: false, status: "error" }}
			/>,
		);
		expect(screen.getByText("52.000,00")).toBeInTheDocument();
	...
	it("states the anchor the cash figure is reconciled to", () => {
		...
		expect(
			screen.getByText("Conciliado al 31/05/2026 · últ. mov. 28/08/2026"),
		).toBeInTheDocument();
	});
	...
	it("accepts an explicit cashMeta for callers that hold it apart from the metrics", () => {
		...
		expect(
			screen.getByText("Conciliado al 30/06/2026 · últ. mov. 01/07/2026"),
		).toBeInTheDocument();
	});
});
